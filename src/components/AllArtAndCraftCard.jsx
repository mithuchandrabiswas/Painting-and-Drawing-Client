...
const AllArtAndCraftCard = ({ singleCraft }) => {
    ...
    return (
        <div className="shadow-sm space-y-1 py-4 px-2 border">
            ...
            <Link to={`/allartandcraftdetails/${_id}`} onClick={() => handleAllViewDetails(singleCraft)} CardDetails={<AllArtAndCraftCardDetails singleCraft={singleCraft}></AllArtAndCraftCardDetails>} className="btn btn-sm btn-secondary w-full">View Details</Link>
        </div>
    );
};
